...
# Уточнення від стейкхолдерів

Первинні вимоги вище подані так, як їх сформулював замовник, і не редагуються. Нижче —
подальші зауваження, у хронологічному порядку; кожне має вести до конкретної зміни у
[02_tech_stack.md](./02_tech_stack.md) та [03_roadmap.md](./03_roadmap.md).

## 2026-07-29 — розмежування видів стягнення

> «В короткому змісті немає розмежування між доганою і суворою доганою.»

Стосується **рішень** (ухвала про відкриття справи стягнення не накладає взагалі).

Перевірка підтвердила зауваження і уточнила його: поле `sanction` розрізняло види стягнення,
але **8 із 9** рішень із доганою/суворою доганою взагалі не називали стягнення у полі
`summary.essence` — тобто саме там, куди дивиться читач. Причина: системний промпт стадії 21
не містив жодної вказівки щодо стягнення.

**Рішення:** (а) нове поле `sanction_type` — вид стягнення за частиною першою статті 109
з фіксованого переліку, поряд із повним формулюванням у `sanction`; (б) вимога у промпті
називати накладене стягнення повністю у `summary.essence`. Догана і сувора догана — різні
стягнення з різними наслідками (позбавлення доплат на один місяць проти трьох), тому це
фасет для фільтрації, а не деталь формулювання. Запроваджується разом із повним прогоном
стадії 21 — див. [M4](./03_roadmap.md#m4--extraction-across-the-corpus--current).
